...
class MetricServiceApi(object):
    ...
    def __init__(self,
        ...
        self._list_time_series = api_callable.create_api_call(
            self.stub.ListTimeSeries, settings=defaults['list_time_series'])
    ...
    def list_monitored_resource_descriptors(self,
        ...
        Example:
          >>> from google.cloud.gapic.monitoring.v3.metric_service_api import MetricServiceApi
          >>> from google.gax import CallOptions, INITIAL_PAGE
          >>> api = MetricServiceApi()
          >>> name = api.project_path('[PROJECT]')
          >>> filter = ''
          >>>
          >>> # Iterate over all results
          >>> for element in api.list_monitored_resource_descriptors(name, filter):
          >>>   # process element
          >>>   pass
          >>>
          >>> # Or iterate over results one page at a time
          >>> for page in api.list_monitored_resource_descriptors(name, filter, options=CallOptions(page_token=INITIAL_PAGE)):
          >>>   for element in page:
          >>>     # process element
          >>>     pass
    ...
    def list_metric_descriptors(self, name, filter_, page_size=0,
        ...
        Example:
          ...
          >>>
          >>> # Iterate over all results
          >>> for element in api.list_metric_descriptors(name, filter):
          >>>   # process element
          >>>   pass
          >>>
          >>> # Or iterate over results one page at a time
          >>> for page in api.list_metric_descriptors(name, filter, options=CallOptions(page_token=INITIAL_PAGE)):
          >>>   for element in page:
          >>>     # process element
          >>>     pass
    ...
    def list_time_series(self,
        ...
        Example:
          ...
          >>> order_by = ''
          >>> view = metric_service_pb2.ListTimeSeriesRequest.TimeSeriesView.FULL
          >>>
          >>> # Iterate over all results
          >>> for element in api.list_time_series(name, filter, interval, aggregation, order_by, view):
          >>>   # process element
          >>>   pass
          >>>
          >>> # Or iterate over results one page at a time
          >>> for page in api.list_time_series(name, filter, interval, aggregation, order_by, view, options=CallOptions(page_token=INITIAL_PAGE)):
          >>>   for element in page:
          >>>     # process element
          >>>     pass
        ...
        Args:
          ...
          view (:class:`google.monitoring.v3.metric_service_pb2.ListTimeSeriesRequest.TimeSeriesView`): Specifies which information is returned about the time series.
          page_size (int): The maximum number of resources contained in the
            underlying API response. If page streaming is performed per-
            resource, this parameter does not affect the return value. If page
            streaming is performed per-page, this determines the maximum number
